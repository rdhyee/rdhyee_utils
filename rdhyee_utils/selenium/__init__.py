...
def selenium_driver(browser='firefox', headless=False, profile=None):
    ...
    CHROMEDRIVER_PATH = (
        '/Users/raymondyee/D/Document/selenium/chromedriver')
        # '/Users/raymondyee/C/src/Gluejar/regluit/test/chromedriver')
    ...
    if browser == 'firefox':
        ...
        driver = webdriver.Firefox(capabilities=firefox_capabilities, executable_path=FIREFOX_PATH, options=firefox_options)
...
def find_element(sel, selector, selector_type='css', timeout=20):
    ...
    if selector_type == 'css':
        return (WebDriverWait(sel, timeout)
                .until(lambda d: d.find_element_by_css_selector(selector)))
    elif selector_type == 'xpath':
        return (WebDriverWait(sel, timeout)
                .until(lambda d: d.find_element_by_xpath(selector)))
...
def find_elements(sel, selector, selector_type='css', timeout=20):
    ...
    if selector_type == 'css':
        return (WebDriverWait(sel, timeout)
                .until(lambda d: d.find_elements_by_css_selector(selector)))
    elif selector_type == 'xpath':
        return (WebDriverWait(sel, timeout)
                .until(lambda d: d.find_elements_by_xpath(selector)))
...
def test_wikimedia_commons(browser='Firefox'):
    ...
    sel.quit()
    assert (commons_link.get_attribute('href') ==
